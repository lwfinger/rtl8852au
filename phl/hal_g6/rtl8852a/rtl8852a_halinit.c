...
u32 _hal_cfg_extnal_fw_8852a(struct rtw_phl_com_t *phl_com,
	enum rtw_fw_type fw_type, struct rtw_fw_info_t *fw_info, char *ic_name)
{
	void *d = phlcom_to_drvpriv(phl_com);
	char *hal_phy_folder = FW_FILE_CONFIG_PATH;
	char *filename_postfix = "";
	static u8 wow_buf[RTW_MAX_FW_SIZE] = {0}, buf[RTW_MAX_FW_SIZE] = {0};
	static u32 wow_buf_size = 0, buf_size = 0;
	u8 *tgt_buf = NULL;
	u32 *tgt_buf_size = NULL;

	switch (fw_type) {
	case RTW_FW_NIC:
		filename_postfix = FW_FILE_NIC_POSTFIX;
		tgt_buf = buf;
		tgt_buf_size = &buf_size;
		break;
	case RTW_FW_WOWLAN:
		filename_postfix = FW_FILE_WOWLAN_POSTFIX;
		tgt_buf = wow_buf;
		tgt_buf_size = &wow_buf_size;
		break;
	case RTW_FW_SPIC:
		filename_postfix = FW_FILE_SPIC_POSTFIX;
		tgt_buf = buf;
	...
	/* fw has not been downloaded yet */
	if (0 == *tgt_buf_size) {
		*tgt_buf_size = _os_read_file(fw_info->ram_path,
									tgt_buf,
									RTW_MAX_FW_SIZE);
	...
	_os_mem_cpy(d, fw_info->ram_buff, tgt_buf, fw_info->ram_size);
				     ...
				     enum rtw_fw_type fw_type)
{
	...
	if (fw_cap->dlrom_en) {
		if (RTW_HAL_STATUS_SUCCESS !=
				_hal_cfg_rom_fw_8852a(fw_type, fw_info, ic_name))
			goto init_fw_fail;
	}
	...
	if (fw_cap->fw_src == RTW_FW_SRC_EXTNAL) {
		...
		if (RTW_HAL_STATUS_SUCCESS !=
				_hal_cfg_extnal_fw_8852a(phl_com, fw_type, fw_info, ic_name))
			goto init_fw_fail;
	} else if (fw_cap->fw_src == RTW_FW_SRC_INTNAL) {
		fw_info->fw_src = RTW_FW_SRC_INTNAL;
		if (RTW_HAL_STATUS_SUCCESS !=
				_hal_cfg_intnal_fw_8852a(phl_com, fw_type, fw_info))
			goto init_fw_fail;
	} else {
		goto init_fw_fail;
	}
