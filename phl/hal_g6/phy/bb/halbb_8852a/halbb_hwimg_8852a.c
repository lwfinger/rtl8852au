...
bool halbb_sel_headline_8852a(struct bb_info *bb, u32 *array, u32 array_len,
...
void halbb_flag_2_default_8852a(bool *is_matched, bool *find_target)
{
	*is_matched = true;
	*find_target = false;
}
			  ...
			  enum phl_phy_idx phy_idx)
{
	...
	if (is_form_folder) {
		...
		array_len = sizeof(array_mp_8852a_phy_reg) / sizeof(u32);
		array = (u32 *)array_mp_8852a_phy_reg;
	}

	BB_DBG(bb, DBG_INIT, "BBCR_form_folder=%d, len=%d, phy_idx=%d\n",
	       is_form_folder, array_len, phy_idx);

	if (!halbb_sel_headline_8852a(bb, array, array_len, &h_size, &h_idx)) {
	...
	halbb_flag_2_default_8852a(&is_matched, &find_target);
	while ((i + 1) < array_len) {
		v1 = array[i];
		v2 = array[i + 1];
		i += 2;
		...
		default:
			if (is_matched) 
				halbb_cfg_bb_phy_8852a(bb, v1, v2, phy_idx);
			break;
		}
	}
...
u32
halbb_get_8852a_phy_reg_ver(void)
{
		return (u32)BB_REG_RELEASE_VERSION_8852A;
}
