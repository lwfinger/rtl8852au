			      ...
			      char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[1], "-h") == 0) {
		...
		BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
			 "pfd tdma en {val}\n");
		BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
			 "pfd tdma period {Lgcy_ms} {All_mode_ms}\n");
		#endif
		return;
	}
	...
	if (_os_strcmp(input[1], "pop_en") == 0) {
		...
		if (_os_strcmp(input[2], "set") == 0) {
			...
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "pfd_fmt_type=%d\n", val[0]);
		#ifdef BB_TDMA_PFD
		} else if (_os_strcmp(input[2], "tdma") == 0) {
			HALBB_SCAN(input[4], DCMD_DECIMAL, &val[0]);
			HALBB_SCAN(input[5], DCMD_DECIMAL, &val[1]);
			if (_os_strcmp(input[3], "en") == 0) {
				bb->bb_8852a_2_i.tdma_pfd_en = (bool)val[0];
				halbb_pkt_fmt_sel_main_8852a_2(bb);
				BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
					    "tdma_pfd_en=%d\n", bb->bb_8852a_2_i.tdma_pfd_en);
			} else if (_os_strcmp(input[3], "period") == 0) {
				bb->bb_8852a_2_i.pfd_period_ms_0 = val[0];
				bb->bb_8852a_2_i.pfd_period_ms_1 = val[1];
				BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
					    "period_ms_0/1= {%d, %d}\n",
					    bb->bb_8852a_2_i.pfd_period_ms_0,
					    bb->bb_8852a_2_i.pfd_period_ms_1);
			}
		#endif
		} 
	#endif
