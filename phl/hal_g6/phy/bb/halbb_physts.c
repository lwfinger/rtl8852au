...
void halbb_physts_detail_dump(struct bb_info *bb, u32 bitmap, u32 bitmap_mask)
{
	...
	if (bitmap & BIT(IE01_CMN_OFDM)) {
		...
		BB_TRACE("[01][OFDM] path_en=0x%x, rpl=%d.%01d, cfo=%s%d.%02d kHz, Max&Min evm={%d.%02d,%d.%02d}, avg SNR=%d , CN=%d.%01d\n",
			 psts_1->rx_path_en,
			 psts_1->rpl_fd >> 1,
			   ...
			   struct physts_rxd *desc)
{
	...
	/* for CAV, using the result of L1/L2 estimation */
	cfo_tmp = (ie_1->avg_cfo_premb_seg0_m << 4) | ie_1->avg_cfo_premb_seg0_l;
	//BB_DBG(bb, DBG_CMN, "cfo_tmp=%d\n", cfo_tmp);
...
PARSING_END:
	...
	if (desc->is_to_self) {
		...
		halbb_parsing_cfo(bb, physts->bb_physts_rslt_1_i.cfo_avg, desc);
		#endif
