...
#else
	buf->ch_info_state = CH_RPT_START_TO_WAIT;
	halbb_cfg_ch_info_en(bb, CH_INFO_FROM_PHY_STS, HE_PKT, bb->bb_phy_idx);
	//Delay for get physts
	halbb_delay_ms(bb, bf->get_phy_sts_dly);
	halbb_cfg_ch_info_en(bb, CH_INFO_DISABLE, HE_PKT, bb->bb_phy_idx);
#endif
...
void halbb_dyn_csi_rsp_main(struct bb_info *bb)
{
	...
	is_he_connect = halbb_dcr_is_he_connect(bb);
	if(!is_he_connect){
