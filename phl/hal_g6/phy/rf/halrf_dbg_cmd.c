	...
	case HALRF_HWTX:
		{
			...
			if (value[0] == 1) {
				...
				halrf_btc_rfk_ntfy(rf, (BIT(HW_BAND_0) << 4), RF_BTC_TSSI, RFK_START);
				halrf_tmac_tx_pause(rf, HW_BAND_0, true);
			...
			if (value[0] == 0) {
				...
				halrf_tx_mode_switch(rf, HW_BAND_0, 0);

				halrf_tmac_tx_pause(rf, HW_BAND_0, false);
				halrf_btc_rfk_ntfy(rf, (BIT(HW_BAND_0) << 4), RF_BTC_TSSI, RFK_STOP);
			}
		}
		break;
	default:
