...
void halrf_check_fwiqk_done(struct rf_info *rf)
{
...
#ifdef RF_8852A_SUPPORT
		case CHIP_WIFI6_8852A:
			halrf_check_fwiqk_done_8852ab(rf);
		break;
#endif

#ifdef RF_8852B_SUPPORT
		case CHIP_WIFI6_8852B:
			halrf_check_fwiqk_done_8852b(rf);
		break;
#endif
		default:
		break;
...
void halrf_fw_iqk(struct rf_info *rf, enum phl_phy_idx phy_idx, bool force) {
	
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	u32 data_to_fw[2] = {0};
	u16 len = (u16) (sizeof(data_to_fw) / sizeof(u32))*4;

	RF_DBG(rf, DBG_RF_IQK, "[IQK]====  FW IQK START v2 ==== \n");
