...
_os_list grx_q;
_os_lock grx_q_lock;
struct rtw_pool gtx_req_pool;
struct rtw_pool grx_req_pool;
struct rtw_pool gtx_pkt_pool;
struct test_obj_ctrl_interface gtrx_test_obj;
struct rtw_trx_test_param gtest_param;
struct rtw_phl_handler gtest_rxq_handler;
...
void _phl_free_rx_req_pool(void *phl)
{
	...
	PHL_INFO("_phl_free_rx_req_pool : idle counter (%d), busy counter (%d), total counter (%d)\n",
			 grx_req_pool.idle_cnt, grx_req_pool.busy_cnt, grx_req_pool.total_cnt);
	_os_spinlock_free(drv_priv, &grx_req_pool.idle_lock);
	_os_spinlock_free(drv_priv, &grx_req_pool.busy_lock);
	INIT_LIST_HEAD(&grx_req_pool.idle_list);
	INIT_LIST_HEAD(&grx_req_pool.busy_list);

	if (NULL != grx_req_pool.buf) {
		_os_mem_free(drv_priv, grx_req_pool.buf, grx_req_pool.buf_len);
	}

	grx_req_pool.buf_len = 0;
	grx_req_pool.idle_cnt = 0;
	grx_req_pool.busy_cnt = 0;
	grx_req_pool.total_cnt = 0;
...
enum rtw_phl_status _phl_alloc_rx_req_pool(void *phl, u32 rx_req_num)
{
	...
	do {
		...
		grx_req_pool.total_cnt = rx_req_num;
		buf_len = sizeof(struct rtw_test_rx) * rx_req_num;
		grx_req_pool.buf = _os_mem_alloc(drv_priv, buf_len);

		if (NULL == grx_req_pool.buf)
			break;
		grx_req_pool.buf_len = buf_len;
		INIT_LIST_HEAD(&grx_req_pool.idle_list);
		INIT_LIST_HEAD(&grx_req_pool.busy_list);
		_os_spinlock_init(drv_priv, &grx_req_pool.idle_lock);
		_os_spinlock_init(drv_priv, &grx_req_pool.busy_lock);
		rreq = (struct rtw_test_rx *)grx_req_pool.buf;
		for (i = 0; i < rx_req_num; i++) {
			INIT_LIST_HEAD(&rreq[i].list);
			list_add_tail(&rreq[i].list, &grx_req_pool.idle_list);
			grx_req_pool.idle_cnt++;
		}
...
struct rtw_test_rx *_phl_query_idle_rx_req(struct phl_info_t *phl_info)
{
	...
	_os_spinlock(drv_priv, &grx_req_pool.idle_lock, _ps, NULL);

	if (false == list_empty(&grx_req_pool.idle_list)) {
		rreq = list_first_entry(&grx_req_pool.idle_list,
				  struct rtw_test_rx, list);
		list_del(&rreq->list);
		grx_req_pool.idle_cnt--;
	}

	_os_spinunlock(drv_priv, &grx_req_pool.idle_lock, _ps, NULL);
...
struct rtw_test_rx *_phl_query_busy_rx_req(struct phl_info_t *phl_info)
{
	...
	_os_spinlock(drv_priv, &grx_req_pool.busy_lock, _ps, NULL);

	if (false == list_empty(&grx_req_pool.busy_list)) {
		rreq = list_first_entry(&grx_req_pool.busy_list,
				  struct rtw_test_rx, list);
		list_del(&rreq->list);
		grx_req_pool.busy_cnt--;
	}

	_os_spinunlock(drv_priv, &grx_req_pool.busy_lock, _ps, NULL);
...
void _phl_release_rx_req(struct phl_info_t *phl_info, struct rtw_test_rx *rreq)
{
	...
	list_add_tail(&rreq->list, &grx_req_pool.idle_list);
	grx_req_pool.idle_cnt++;
...
void _phl_insert_busy_rx_req(struct phl_info_t *phl_info, struct rtw_test_rx *rreq)
{
	...
	list_add_tail(&rreq->list, &grx_req_pool.busy_list);
	grx_req_pool.busy_cnt++;
...
void _phl_free_tx_req_pool(void *phl)
{
	...
	PHL_INFO("_phl_free_tx_req_pool : idle counter (%d), busy counter (%d), total counter (%d)\n",
			 gtx_req_pool.idle_cnt, gtx_req_pool.busy_cnt, gtx_req_pool.total_cnt);
	_os_spinlock_free(drv_priv, &gtx_req_pool.idle_lock);
	_os_spinlock_free(drv_priv, &gtx_req_pool.busy_lock);
	INIT_LIST_HEAD(&gtx_req_pool.idle_list);
	INIT_LIST_HEAD(&gtx_req_pool.busy_list);

	if (NULL != gtx_req_pool.buf) {
		_os_mem_free(drv_priv, gtx_req_pool.buf, gtx_req_pool.buf_len);
	}

	gtx_req_pool.buf_len = 0;
	gtx_req_pool.idle_cnt = 0;
	gtx_req_pool.busy_cnt = 0;
	gtx_req_pool.total_cnt = 0;
...
enum rtw_phl_status _phl_alloc_tx_req_pool(void *phl, u32 tx_req_num)
{
	...
	do {
		...
		gtx_req_pool.total_cnt = tx_req_num;
		buf_len = sizeof(struct rtw_xmit_req) * tx_req_num;
		gtx_req_pool.buf = _os_mem_alloc(drv_priv, buf_len);

		if (NULL == gtx_req_pool.buf)
			break;
		gtx_req_pool.buf_len = buf_len;
		INIT_LIST_HEAD(&gtx_req_pool.idle_list);
		INIT_LIST_HEAD(&gtx_req_pool.busy_list);
		_os_spinlock_init(drv_priv, &gtx_req_pool.idle_lock);
		_os_spinlock_init(drv_priv, &gtx_req_pool.busy_lock);
		treq = (struct rtw_xmit_req *)gtx_req_pool.buf;
		for (i = 0; i < tx_req_num; i++) {
			INIT_LIST_HEAD(&treq[i].list);
			treq[i].treq_type = RTW_PHL_TREQ_TYPE_TEST_PATTERN;
			list_add_tail(&treq[i].list, &gtx_req_pool.idle_list);
			gtx_req_pool.idle_cnt++;
		}
...
struct rtw_xmit_req *_phl_query_idle_tx_req(struct phl_info_t *phl_info)
{
	...
	_os_spinlock(drv_priv, &gtx_req_pool.idle_lock, _ps, NULL);

	if (false == list_empty(&gtx_req_pool.idle_list)) {
		treq = list_first_entry(&gtx_req_pool.idle_list,
				  struct rtw_xmit_req, list);
		list_del(&treq->list);
		gtx_req_pool.idle_cnt--;
	}

	_os_spinunlock(drv_priv, &gtx_req_pool.idle_lock, _ps, NULL);
...
struct rtw_xmit_req *_phl_query_busy_tx_req(struct phl_info_t *phl_info)
{
	...
	_os_spinlock(drv_priv, &gtx_req_pool.busy_lock, _ps, NULL);

	if (false == list_empty(&gtx_req_pool.busy_list)) {
		treq = list_first_entry(&gtx_req_pool.busy_list,
				  struct rtw_xmit_req, list);
		list_del(&treq->list);
		gtx_req_pool.busy_cnt--;
	}

	_os_spinunlock(drv_priv, &gtx_req_pool.busy_lock, _ps, NULL);
...
void _phl_release_tx_req(struct phl_info_t *phl_info, struct rtw_xmit_req *treq)
{
	...
	list_add_tail(&treq->list, &gtx_req_pool.idle_list);
	gtx_req_pool.idle_cnt++;
...
void _phl_insert_busy_tx_req(struct phl_info_t *phl_info, struct rtw_xmit_req *treq)
{
	...
	_os_spinlock(drv_priv, &gtx_req_pool.busy_lock, _ps, NULL);
	list_add_tail(&treq->list, &gtx_req_pool.busy_list);
	gtx_req_pool.busy_cnt++;
...
void _phl_free_tx_pkt_pool(void *phl)
{
	...
	PHL_INFO("_phl_free_tx_pkt_pool : idle counter (%d), busy counter (%d), total counter (%d)\n",
			 gtx_pkt_pool.idle_cnt, gtx_pkt_pool.busy_cnt, gtx_pkt_pool.total_cnt);
	_os_spinlock_free(drv_priv, &gtx_pkt_pool.idle_lock);
	_os_spinlock_free(drv_priv, &gtx_pkt_pool.busy_lock);
	INIT_LIST_HEAD(&gtx_pkt_pool.idle_list);
	INIT_LIST_HEAD(&gtx_pkt_pool.busy_list);

	tpkt = (struct rtw_payload *)gtx_pkt_pool.buf;
	for (i = 0; i <	gtx_pkt_pool.total_cnt; i++) {
	...
	if (NULL != gtx_pkt_pool.buf) {
		_os_mem_free(drv_priv, gtx_pkt_pool.buf, gtx_pkt_pool.buf_len);
	}

	gtx_pkt_pool.buf_len = 0;
	gtx_pkt_pool.idle_cnt = 0;
	gtx_pkt_pool.busy_cnt = 0;
	gtx_pkt_pool.total_cnt = 0;
					   ...
					   u32 tx_pkt_size)
{
	...
	do {
		...
		gtx_pkt_pool.total_cnt = tx_pkt_num;
		buf_len = sizeof(struct rtw_payload) * tx_pkt_num;
		gtx_pkt_pool.buf = _os_mem_alloc(drv_priv, buf_len);

		if (NULL == gtx_pkt_pool.buf)
			break;
		gtx_pkt_pool.buf_len = buf_len;
		INIT_LIST_HEAD(&gtx_pkt_pool.idle_list);
		INIT_LIST_HEAD(&gtx_pkt_pool.busy_list);
		_os_spinlock_init(drv_priv, &gtx_pkt_pool.idle_lock);
		_os_spinlock_init(drv_priv, &gtx_pkt_pool.busy_lock);
		tpkt = (struct rtw_payload *)gtx_pkt_pool.buf;
		for (i = 0; i < tx_pkt_num; i++) {
			...
			list_add_tail(&tpkt[i].list, &gtx_pkt_pool.idle_list);
			gtx_pkt_pool.idle_cnt++;
		}
...
struct rtw_payload *_phl_query_idle_tx_pkt(struct phl_info_t *phl_info)
{
	...
	_os_spinlock(drv_priv, &gtx_pkt_pool.idle_lock, _ps, NULL);

	if (false == list_empty(&gtx_pkt_pool.idle_list)) {
		tpkt = list_first_entry(&gtx_pkt_pool.idle_list,
				  struct rtw_payload, list);
		list_del(&tpkt->list);
		gtx_pkt_pool.idle_cnt--;
	}

	_os_spinunlock(drv_priv, &gtx_pkt_pool.idle_lock, _ps, NULL);
...
struct rtw_payload *_phl_query_busy_tx_pkt(struct phl_info_t *phl_info)
{
	...
	_os_spinlock(drv_priv, &gtx_pkt_pool.busy_lock, _ps, NULL);

	if (false == list_empty(&gtx_pkt_pool.busy_list)) {
		tpkt = list_first_entry(&gtx_pkt_pool.busy_list,
				  struct rtw_payload, list);
		list_del(&tpkt->list);
		gtx_pkt_pool.busy_cnt--;
	}

	_os_spinunlock(drv_priv, &gtx_pkt_pool.busy_lock, _ps, NULL);
...
void _phl_release_tx_pkt(struct phl_info_t *phl_info, struct rtw_payload *tpkt)
{
	...
	INIT_LIST_HEAD(&tpkt->list);
	list_add_tail(&tpkt->list, &gtx_pkt_pool.idle_list);
	gtx_pkt_pool.idle_cnt++;
...
void _phl_insert_busy_tx_pkt(struct phl_info_t *phl_info, struct rtw_payload *tpkt)
{
	...
	list_add_tail(&tpkt->list, &gtx_pkt_pool.busy_list);
	gtx_pkt_pool.busy_cnt++;
...
{
	if (list_empty(&gtx_req_pool.busy_list) && list_empty(&gtx_pkt_pool.busy_list))
		return true;
	else
		return false;
}
...
void phl_update_test_param(void *phl, struct rtw_trx_test_param *test_param)
{
	...
	_os_mem_cpy(drv_priv, &gtest_param, test_param, sizeof(*test_param));
}
...
void phl_trx_test_init(void *phl)
{
	...
	INIT_LIST_HEAD(&grx_q);
	_os_mem_set(drv_priv, &gtx_req_pool, 0, sizeof(gtx_req_pool));
	_os_mem_set(drv_priv, &grx_req_pool, 0, sizeof(grx_req_pool));
	_os_mem_set(drv_priv, &gtx_pkt_pool, 0, sizeof(gtx_pkt_pool));
	_os_mem_set(drv_priv, &gtest_param, 0, sizeof(gtest_param));
	_os_mem_set(drv_priv, &gtrx_test_obj, 0, sizeof(gtrx_test_obj));
	ops->tx_test_recycle = phl_recycle_test_tx;
	phl_test_sw_alloc(phl);
#if 0
...
void rtw_phl_test_rx_callback(void *context)
{
	...
	while (!list_empty(&grx_req_pool.busy_list)) {
				   ...
				   struct rtw_trx_test_param *test_param)
{
	...
	FUNCIN_WSTS(status);
	do {
		gtest_param.is_trx_test_end = false;
		...
		gtest_param.is_trx_test_end = true;
	} while (false);
	FUNCOUT_WSTS(status);
	return status;
}
...
u8 trx_test_is_test_end(void *priv)
{
	...
	FUNCIN();
	FUNCOUT();
	return (gtest_param.is_trx_test_end == true) ? (true) : (false);
}
...
phl_add_trx_test_obj(void *phl)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	FUNCIN();
	gtrx_test_obj.bp_handler = trx_test_bp_handler;
	gtrx_test_obj.get_fail_rsn = trx_test_get_fail_rsn;
	gtrx_test_obj.is_test_end = trx_test_is_test_end;
	gtrx_test_obj.is_test_pass = trx_test_is_test_pass;
	gtrx_test_obj.start_test = trx_test_start_test;
